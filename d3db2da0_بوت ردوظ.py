...
def get_admin_panel_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    if has_permission(user_id, "can_view_stats") or is_owner(user_id):
        row.append(StyButton.create("الإحصائيات", callback_data="admin_stats", style="info"))
...
def load_keywords_from_file() -> Dict[str, str]:
    ...
    try:
        ...
        if not lines:
            return {}
        # تنسيق الملف: كل سطر يحتوي على كلمات مفصولة بفواصل ثم "|" ثم الرد
        # مثال: انستا,انستاغرام,انستغرام,انست | هذا هو الرد
        result = {}
        for line in lines:
            if '|' not in line:
                continue
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    verse = format_verse()
    welcome_text = (
        f"<b>🌟 مرحباً بك في بوت إدارة المجموعات</b>\n\n"
        f"<blockquote>هذا البوت مصمم خصيصاً لإدارة مجموعاتك بشكل احترافي.\nيمكنك استخدام الأزرار أدناه للتنقل.</blockquote>\n\n"
