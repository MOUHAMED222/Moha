...
# ------------------ توكن البوت ------------------
BOT_TOKEN = "8833337494:AAEv6Y4X1Jh140GCqDto3z3KN2rpF8-3GkA"  # استبدل بالتوكن الخاص بك

# ------------------ معرف المالك (أنت) ------------------
OWNER_ID = 6891530912  # استبدل بمعرفك الرقمي

# ------------------ الروابط الأساسية ------------------
CHANNEL_LINK = "https://t.me/forzd9"       # قناتك الأساسية
GROUP_LINK = "https://t.me/maamihooo"           # مجموعتك الأساسية
SUPPORT_LINK = "https://t.me/mouhamed_ma"       # رابط الدعم
DEVELOPER_LINK = "https://t.me/mouhamed_ma"         # رابط المطور

# ------------------ صورة الترحيب ------------------
WELCOME_IMAGE_URL = "https://d.top4top.io/p_38534mjfh0.jpg"  # رابط الصورة
...
# ------------------ قائمة الآيات القرآنية (اختيار عشوائي) ------------------
VERSES = [
...
# =====================================================
# ================ طبقة مساعدة للأزرار (Style) ==========
# =====================================================

class StyledButton:
    """
    مصنع للأزرار مع دعم style (success, danger, primary)
    يقوم بإضافة إيموجي مناسب أمام النص حسب النمط.
    """
    STYLE_MAP = {
        "success": "✅",
        "danger": "❌",
    ...
    def create(
        ...
        """إنشاء زر مع نمط محدد."""
        # إضافة الإيموجي حسب النمط
        emoji = StyledButton.STYLE_MAP.get(style, "⚪")
        styled_text = f"{emoji} {text}" if not text.startswith(emoji) else text
...
class Database:
    """التحكم في قاعدة البيانات SQLite."""
    ...
    def _init_db(self):
        ...
        with self._connect() as conn:
            ...
            # جدول الأدمن (من يمكنهم استخدام لوحة الأدمن)
            cursor.execute("""
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS auto_replies (
                    ...
                    reply_type TEXT DEFAULT 'text',  -- text, photo, video, audio, document
                    reply_content TEXT,              -- النص أو معرف الملف
                    caption TEXT,
            ...
            # جدول الاشتراك الإجباري (القنوات)
            cursor.execute("""
            ...
            # جدول النسخ الاحتياطية (مسارات الملفات)
            cursor.execute("""
            ...
            # إضافة المستخدم المالك في جدول users إن لم يكن
            cursor.execute(
                "INSERT OR IGNORE INTO users (user_id, is_owner, joined_date) VALUES (?, ?, ?)",
                (OWNER_ID, 1, datetime.now().isoformat())
            )
    ...
    def get_or_create_user(self, user: User) -> Dict:
        """إرجاع بيانات المستخدم، وإنشاؤه إذا لم يكن موجوداً."""
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def add_admin(
        ...
        """إضافة أدمن جديد مع صلاحيات محددة."""
...
def get_main_keyboard() -> InlineKeyboardMarkup:
    """لوحة المفاتيح الرئيسية للبوت."""
    buttons = [
        [
            StyledButton.create("القناة الخاصة", f"open_{CHANNEL_LINK}", "primary"),
            StyledButton.create("المجموعة", f"open_{GROUP_LINK}", "primary"),
        ],
        [
            StyledButton.create("الدعم", f"open_{SUPPORT_LINK}", "info"),
            StyledButton.create("المطور", f"open_{DEVELOPER_LINK}", "info"),
        ],
        [
            StyledButton.create("الاشتراك الإجباري", "forced_subscription", "warning"),
            StyledButton.create("إضافة في مجموعة", "add_to_group", "success"),
        ],
        [
            StyledButton.create("لوحة الأدمن", "admin_panel", "danger"),
        ],
    ]
    return InlineKeyboardMarkup(buttons)

def get_admin_panel_keyboard(user_id: int) -> InlineKeyboardMarkup:
    """لوحة الأدمن الرئيسية حسب الصلاحيات."""
    buttons = []

    # إدارة الردود
    if has_permission(user_id, "can_manage_replies") or is_owner(user_id):
        buttons.append([
            StyledButton.create("إدارة الردود", "admin_replies", "primary"),
        ])

    # إدارة المجموعات
    if has_permission(user_id, "can_manage_groups") or is_owner(user_id):
        buttons.append([
            StyledButton.create("إدارة المجموعات", "admin_groups", "primary"),
        ])

    # إدارة الأدمن
    if has_permission(user_id, "can_manage_admins") or is_owner(user_id):
        buttons.append([
            StyledButton.create("إدارة الأدمن", "admin_admins", "primary"),
        ])

    # النشر والإحصائيات
    if has_permission(user_id, "can_broadcast") or is_owner(user_id):
        buttons.append([
            StyledButton.create("نشر رسالة", "admin_broadcast", "primary"),
        ])
    if has_permission(user_id, "can_view_stats") or is_owner(user_id):
        buttons.append([
            StyledButton.create("الإحصائيات", "admin_stats", "info"),
        ])

    # الإعدادات والنسخ الاحتياطي
    if has_permission(user_id, "can_manage_settings") or is_owner(user_id):
        buttons.append([
            StyledButton.create("الإعدادات", "admin_settings", "secondary"),
        ])
    if has_permission(user_id, "can_backup") or is_owner(user_id):
        buttons.append([
            StyledButton.create("النسخ الاحتياطي", "admin_backup", "secondary"),
        ])

    # سجل العمليات
    if has_permission(user_id, "can_view_stats") or is_owner(user_id):
        buttons.append([
            StyledButton.create("سجل العمليات", "admin_logs", "info"),
        ])

    buttons.append([
        StyledButton.create("الرجوع للقائمة الرئيسية", "main_menu", "secondary"),
    ])
...
def get_back_button(callback: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [StyledButton.create("رجوع", callback, "secondary")]
    ])
    ...
    if not_subscribed:
        ...
        for ch in not_subscribed:
            ...
            if username:
                ...
                link = f"https://t.me/{ch['channel_id']}"  # قد لا يعمل
            buttons.append([
                InlineKeyboardButton(
        ...
        buttons.append([
            StyledButton.create("تحقق من الاشتراك", "check_subscription", "success")
        ])
        keyboard = InlineKeyboardMarkup(buttons)

        # إرسال رسالة إذا كانت الرسالة في الخاص
...
async def open_link_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """فتح رابط (يستخدم مع الأزرار التي تحتوي على url)."""
    query = update.callback_query
    await query.answer("جاري فتح الرابط...", show_alert=False)

async def forced_subscription_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """عرض إعدادات الاشتراك الإجباري."""
    query = update.callback_query
    ...
    buttons = [
        [StyledButton.create("➕ إضافة قناة", "add_forced_channel", "success")],
        [StyledButton.create("🗑️ حذف قناة", "remove_forced_channel", "danger")],
        [StyledButton.create("🔄 تبديل تفعيل قناة", "toggle_forced_channel", "warning")],
        [StyledButton.create("رجوع", "admin_panel", "secondary")],
    ]
    keyboard = InlineKeyboardMarkup(buttons)
    await query.edit_message_text(text, reply_markup=keyboard, parse_mode=ParseMode.MARKDOWN)

async def add_forced_channel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """بدء إضافة قناة إجبارية (طلب إدخال ID)."""
    query = update.callback_query
    await query.answer()
    await query.edit_message_text(
...
async def remove_forced_channel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons.append([
        StyledButton.create("رجوع", "forced_subscription", "secondary")
    ])
    await query.edit_message_text(
...
async def toggle_forced_channel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """عرض قائمة القنوات لتبديل التفعيل."""
    query = update.callback_query
    await query.answer()
    channels = db.get_forced_channels(active_only=False)
    if not channels:
...
async def admin_replies_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if replies:
        for r in replies:
            ...
            text += f"- {r['keyword']} (ID: {r['id']}) - {status}\n"
    else:
    ...
    buttons = [
        [StyledButton.create("➕ إضافة رد جديد", "add_reply_start", "success")],
        [StyledButton.create("✏️ تعديل رد", "edit_reply_start", "primary")],
        [StyledButton.create("🗑️ حذف رد", "delete_reply_start", "danger")],
        [StyledButton.create("🔄 تبديل تفعيل رد", "toggle_reply_start", "warning")],
        [StyledButton.create("رجوع", "admin_panel", "secondary")],
    ]
    await query.edit_message_text(
        text,
...
async def add_reply_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "أرسل الكلمة المفتاحية (مثال: انستا) ثم اتبع التعليمات.\n"
        "لإلغاء العملية اضغط /cancel.",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def edit_reply_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in replies:
        buttons.append([
            InlineKeyboardButton(
                text=f"✏️ {r['keyword']} (ID: {r['id']})",
                callback_data=f"edit_reply_{r['id']}"
            )
        ])
    buttons.append([
        StyledButton.create("رجوع", "admin_replies", "secondary")
    ])
    await query.edit_message_text(
...
async def edit_reply_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "✏️ *تعديل رد*\n\n"
        "أرسل الكلمة المفتاحية الجديدة، أو اكتب 'نفس' للإبقاء على نفس الكلمة.\n"
...
async def toggle_reply_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in replies:
        ...
        buttons.append([
            InlineKeyboardButton(
                text=f"{status} {r['keyword']} (ID: {r['id']})",
...
async def admin_groups_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        [StyledButton.create("🔄 تبديل تفعيل مجموعة", "toggle_group_start", "warning")],
        [StyledButton.create("🗑️ حذف مجموعة", "delete_group_start", "danger")],
        [StyledButton.create("📊 عرض كل المجموعات", "list_groups", "info")],
        [StyledButton.create("رجوع", "admin_panel", "secondary")],
    ]
    await query.edit_message_text(
        text,
...
async def toggle_group_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons.append([
        StyledButton.create("رجوع", "admin_groups", "secondary")
    ])
    await query.edit_message_text(
...
async def admin_admins_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        [StyledButton.create("➕ إضافة أدمن", "add_admin_start", "success")],
        [StyledButton.create("🗑️ حذف أدمن", "remove_admin_start", "danger")],
        [StyledButton.create("📋 عرض الأدمن", "list_admins", "info")],
        [StyledButton.create("رجوع", "admin_panel", "secondary")],
    ]
    await query.edit_message_text(
        text,
...
async def remove_admin_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons.append([
        StyledButton.create("رجوع", "admin_admins", "secondary")
    ])
    await query.edit_message_text(
        "اختر الأدمن لحذفه:",
        reply_markup=InlineKeyboardMarkup(buttons),
...
async def admin_broadcast_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    buttons = [
        [StyledButton.create("📨 نشر للجميع (خاص)", "broadcast_users", "primary")],
        [StyledButton.create("📢 نشر للمجموعات", "broadcast_groups", "primary")],
        [StyledButton.create("📢 نشر لمجموعة محددة", "broadcast_specific_group", "primary")],
        [StyledButton.create("رجوع", "admin_panel", "secondary")],
    ]
    await query.edit_message_text(
        "📨 *لوحة النشر*\nاختر وجهة النشر:",
...
async def broadcast_specific_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons.append([
        StyledButton.create("رجوع", "admin_broadcast", "secondary")
    ])
    await query.edit_message_text(
...
async def admin_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        [StyledButton.create("تعديل رابط القناة", "set_channel_link", "primary")],
        [StyledButton.create("تعديل رابط المجموعة", "set_group_link", "primary")],
        [StyledButton.create("تعديل رابط الدعم", "set_support_link", "primary")],
        [StyledButton.create("تعديل رابط المطور", "set_developer_link", "primary")],
        [StyledButton.create("تعديل صورة الترحيب", "set_welcome_image", "primary")],
        [StyledButton.create("تعديل نص الترحيب", "set_welcome_text", "primary")],
        [StyledButton.create("رجوع", "admin_panel", "secondary")],
    ]
    await query.edit_message_text(
        text,
...
async def admin_backup_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        [StyledButton.create("📤 إنشاء نسخة احتياطية", "create_backup", "success")],
        [StyledButton.create("📥 استعادة نسخة", "restore_backup_start", "warning")],
        [StyledButton.create("رجوع", "admin_panel", "secondary")],
    ]
    await query.edit_message_text(
        text,
...
async def restore_backup_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons.append([
        StyledButton.create("رجوع", "admin_backup", "secondary")
    ])
    await query.edit_message_text(
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالجة الرسائل النصية (للاستعلامات والردود)."""
    user = update.effective_user
    if not user:
        return

    # التحقق من الاشتراك الإجباري للرسائل في الخاص
    if update.message and update.message.chat.type == "private":
        if not await check_forced_subscription(update, context):
            return

    # معالجة إجراءات المستخدم (مثل إضافة رد أو إعدادات)
    ...
    if action == "add_forced_channel" and update.message:
        ...
        try:
            # محاولة تحويل إلى int (ID)
            if text.startswith("-100"):
                channel_id = int(text)
            elif text.startswith("@"):
                # محاولة الحصول على ID عن طريق الحصول على chat
                bot = context.bot
                try:
    ...
    if action == "add_reply_keyword" and update.message:
    ...
    if action == "add_reply_content" and update.message:
        ...
        success = db.add_reply(keyword, reply_type, content, caption, "HTML", user.id)
        if success:
            db.add_log(user.id, "إضافة رد تلقائي", keyword, f"النوع: {reply_type}")
            await update.message.reply_text(f"✅ تم إضافة الرد للكلمة '{keyword}' بنجاح.")
        else:
    ...
    if action == "broadcast_users" and update.message:
        ...
        # هنا يمكن جلب جميع المستخدمين من قاعدة البيانات (لكن قد يكون عدد كبير)
        # سنحصل على المستخدمين من جدول users (فقط غير البوت)
        ...
        for u in users:
            try:
                ...
                count += 1
                await asyncio.sleep(0.05)  # تجنب السبام
            except Exception as e:
    ...
    # الردود التلقائية في المجموعات
    if update.message and update.message.chat.type in ["group", "supergroup"]:
        text = update.message.text
        if text:
            # التحقق من الاشتراك الإجباري (للمجموعات لا نطبق اشتراك إجباري)
...
# =====================================================
# ========== معالج إضافة البوت لمجموعة جديدة ============
# =====================================================
...
async def add_to_group_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    # عرض رابط دعوة البوت مع صلاحية الأدمن
    bot_username = (await context.bot.get_me()).username
    invite_link = f"https://t.me/{bot_username}?startgroup=add"
    await query.edit_message_text(
        "🤖 *إضافة البوت إلى مجموعة جديدة*\n\n"
        ...
        f"[اضغط هنا لدعوة البوت]({invite_link})\n\n"
        "أو استخدم معرف البوت: @{bot_username}\n\n"
        "بعد إضافة البوت كأدمن، سيعمل تلقائياً.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("📎 رابط الدعوة", url=invite_link)],
            [StyledButton.create("✅ تمت الإضافة (تحقق)", "check_added_group", "success")],
            [StyledButton.create("رجوع", "main_menu", "secondary")],
        ]),
        parse_mode=ParseMode.MARKDOWN,
    )

async def check_added_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """التحقق من وجود البوت في مجموعات جديدة (يتم تشغيله تلقائياً عند إضافته)."""
    query = update.callback_query
    await query.answer()
    # نقوم بجلب مجموعات البوت من التيليغرام
    bot = context.bot
    try:
        # لا توجد طريقة مباشرة لجلب كل المجموعات، لكننا نستقبل التحديثات عند إضافته.
        # هنا نعرض رسالة بأن البوت سيتفعل تلقائياً عند إضافته.
        await query.edit_message_text(
            "📢 *تم التفعيل التلقائي*\n\n"
...
# =====================================================
# ========== معالج إضافة البوت لمجموعة (تلقائي) =========
# =====================================================

async def my_chat_member_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالج تغيير حالة البوت في المجموعات."""
    ...
    # إذا تم إضافة البوت إلى مجموعة أو تغيير صلاحياته
...
async def send_welcome_message(update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    """إرسال رسالة الترحيب عند التفعيل."""
    bot = context.bot
    try:
        # صورة الترحيب
        ...
        buttons = [
            ...
            [
                InlineKeyboardButton("👥 المجموعة", url=GROUP_LINK),
                InlineKeyboardButton("🆘 الدعم", url=SUPPORT_LINK),
            ],
        ]
        keyboard = InlineKeyboardMarkup(buttons)

        # إرسال الصورة مع الكابتشن والأزرار
        await bot.send_photo(
            chat_id=chat_id,
            photo=welcome_image,
...
def main():
    ...
    # معالج الاستعلامات (callback_query)
    application.add_handler(CallbackQueryHandler(main_menu_callback, pattern="^main_menu$"))
    application.add_handler(CallbackQueryHandler(open_link_callback, pattern="^open_"))
