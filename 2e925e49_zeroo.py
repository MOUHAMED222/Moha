...
# تم إزالة استدعاء telegram لتجنب التعارض لأننا نستخدم API مباشر عبر aiohttp
import yt_dlp
import instaloader
from bs4 import BeautifulSoup
import requests

# محاولة استدعاء generate_user_agent، وإذا لم تكن موجودة نستخدم وكيل افتراضي
try:
...
TOKEN = "8620893904:AAEInQU4PNDA7CR2xiKdtTQNQmC9D2mQ9O4"
CHANNEL_USERNAME = "@ill_ss"
CHANNEL_USERNAME2 = "@V_V_02"
DEVELOPER_IDS = [6519770809]
...
class TelegramAPI:
    ...
    async def _post(self, method: str, **kwargs):
        ...
        async with self.session.post(url, **kwargs) as resp:
            try:
                ...
                logger.error(f"Telegram API response not JSON: {text}")
                return {}
    ...
    async def send_video(self, chat_id, video, caption=None, supports_streaming=True,
        ...
        # إذا كان الفيديو رابطاً (URL) أو File ID نرسله كـ JSON
        ...
        # إذا كان الفيديو ملف (Bytes أو File Object) نرسله باستخدام FormData
        else:
            ...
            if width: form.add_field('width', str(width))
            if height: form.add_field('height', str(height))
            if duration: form.add_field('duration', str(duration))
            if reply_markup:
                form.add_field('reply_markup', json.dumps(reply_markup))
            
            if isinstance(video, bytes):
                form.add_field('video', video, filename='video.mp4', content_type='video/mp4')
            else:
                form.add_field('video', video, filename=getattr(video, 'name', 'video.mp4'))
                
            return await self._post('sendVideo', data=form)
    ...
    async def send_document(self, chat_id, document, caption=None, filename=None, reply_markup=None):
        ...
        if isinstance(document, bytes):
            form.add_field('document', document, filename=filename or 'file.bin')
        else:
            form.add_field('document', document, filename=getattr(document, 'name', 'file.bin'))
            
        return await self._post('sendDocument', data=form)
...
class DownloadBot:
    ...
    # ============================
    # KEYBOARD BUILDERS (Using raw dicts)
    # ============================

    def _build_button(self, btn_def: dict) -> dict:
        btn = {'text': btn_def['text']}
        if 'callback_data' in btn_def:
            btn['callback_data'] = btn_def['callback_data']
        elif 'url' in btn_def:
            btn['url'] = btn_def['url']
        return btn
    ...
    def _main_keyboard(self):
        rows = [
            [
                {'text': 'يوتيوب', 'callback_data': 'download_youtube'},
                {'text': 'فيسبوك', 'callback_data': 'download_facebook'}
            ],
            [
                {'text': 'انستجرام', 'callback_data': 'download_instagram'},
                {'text': 'بينتيريست', 'callback_data': 'download_pinterest'}
            ],
            [
                {'text': 'تيك توك', 'callback_data': 'download_tiktok'},
                {'text': 'سناب شات', 'callback_data': 'download_snapchat'}
            ],
            [
                {'text': 'المطور', 'url': 'https://t.me/mouhamed_ma'},
                {'text': 'القناة', 'url': 'https://t.me/forzd9'}
            ]
        ]
        if DEVELOPER_IDS:
            rows.append([{'text': 'لوحة الأدمن', 'callback_data': 'admin_panel'}])
        
        return {"inline_keyboard": [[self._build_button(btn) for btn in row] for row in rows]}
    ...
    def _admin_keyboard(self):
        rows = [
            ...
            [
                {'text': 'المحظورين', 'callback_data': 'admin_banned_list'},
    ...
    async def _pinterest(self, chat_id, url):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Pinterest IMG error: {e}")
    ...
    async def _instagram(self, chat_id, url):
        ...
        try:
            ydl_opts = {
                'format': 'best',
                'quiet': True,
                'outtmpl': f'instagram_{chat_id}_%(id)s.%(ext)s',
                'socket_timeout': 15,
    ...
    async def _youtube(self, chat_id, url):
        ...
        try:
            ydl_opts = {
                'format': 'best',
                'quiet': True,
                'outtmpl': f'youtube_{chat_id}_%(id)s.%(ext)s',
                'socket_timeout': 15,
                'no_check_certificate': True,
            }
            def dl():
