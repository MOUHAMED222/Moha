...
class StyledButton:
    ...
    لا يضيف إيموجي، بل فقط يضيف النص كما هو.
    """
    @staticmethod
    def create(
        text: str,
        ...
        """إنشاء زر مع نمط محدد (يستخدم فقط للتنظيم)."""
        if url:
...
from datetime import datetime  # يجب أن يكون في أعلى الملف خارج الدالة
import sqlite3
import logging

# باقي المتغيرات كما هي (BOT_TOKEN, OWNER_ID, CHANNEL_LINK ... إلخ)
...
class Database:
    ...
    def _init_db(self):
        with self._connect() as conn:
            ...
            # ===== التعديل الأساسي: التعامل مع المالكين (OWNER_ID) بشكل صحيح =====
            # إدراج كل مالك في جدول المشرفين مع صلاحيات كاملة
            for owner in OWNER_ID:
                # إدراج المالك في جدول admins (إذا لم يكن موجوداً)
                cursor.execute(
                    "INSERT OR IGNORE INTO admins (user_id, added_by, added_date) VALUES (?, ?, ?)",
                    (owner, owner, datetime.now().isoformat())
                )
                # تحديث جميع صلاحياته إلى 1
                cursor.execute("""
                    UPDATE admins SET
                    ...
                    WHERE user_id = ?
                """, (owner,))
                # إدراج المالك في جدول users مع is_owner = 1
                cursor.execute(
...
def get_main_keyboard() -> InlineKeyboardMarkup:
    """لوحة المفاتيح الرئيسية (كل صف يحتوي على زرين)."""
    buttons = [
        [
...
def get_admin_panel_keyboard(user_id: int) -> InlineKeyboardMarkup:
    """لوحة الأدمن (كل صف يحتوي على زرين إن أمكن)."""
    buttons = []
    row = []
...
async def edit_message_safely(
    ...
    """تعديل الرسالة سواء كانت نصية أو تحتوي على وسائط."""
